...
void Floor::Display()
{
	...
	glPushMatrix();
		...
		float bDiffuse[] = { 0.1f, 0.1f, 0.1f, 1.0f };
		float wDiffuse[] = { 0.1f, 0.1f, 0.1f, 1.0f };
		float specular[] = { 1.0f, 1.0f, 1.0f, 1.0f };
		float shininess = 128.0f;

		//rotate 90 degrees on the x axis so it becomes a wall perpendicular to the view direction
		glRotatef(90.0f, 1.0, 0.f, 0.f);
		...
		for (int i = -10; i < 10; i++)
		{
			for (int j = -10; j < 10; j++)
			{
				...
				glVertex3f(scale[0] * static_cast<float>(i) + scale[0], -60.0f, scale[2] * static_cast<float>(j) + scale[2]);
				glVertex3f(scale[0] * static_cast<float>(i) + scale[0], -60.0f, scale[2] * static_cast<float>(j));
				glVertex3f(scale[0] * static_cast<float>(i), -60.0f, scale[2] * static_cast<float>(j));
				glVertex3f(scale[0] * static_cast<float>(i), -60.0f, scale[2] * static_cast<float>(j) + scale[2]);
			}
		}
		glEnd();
